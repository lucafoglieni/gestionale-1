...
async function handleSubscriptionCreated(supabaseClient: any, body: any) {
  ...
  try {
    ...
    console.log("Subscription metadata:", body.data.metadata);
    ...
    if (!userId) {
      ...
      return new Response(
        JSON.stringify({ error: "No user ID found in metadata" }),
        {
          status: 400,
    ...
    const { data, error } = await supabaseClient
      ...
      .insert({
        ...
        user_id: userId, // This will be cast to UUID by the database
...
serve(async (req) => {
  ...
  try {
    ...
    const supabaseClient = createClient(
      ...
      Deno.env.get("SUPABASE_SERVICE_KEY") ?? "", // Use SUPABASE_SERVICE_KEY instead of SUPABASE_SERVICE_ROLE_KEY
    );
    ...
    if (eventId) {
      try {
        const supabaseClient = createClient(
          Deno.env.get("SUPABASE_URL") ?? "",
          Deno.env.get("SUPABASE_SERVICE_KEY") ?? "", // Use SUPABASE_SERVICE_KEY instead
        );
